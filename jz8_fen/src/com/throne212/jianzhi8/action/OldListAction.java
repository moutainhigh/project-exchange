...
private Logger logger = Logger.getLogger(this.getClass());
	...
	private void testLog(){
		...
		logger.debug("jobdate=[" + jobdate + "]");
		logger.debug("unitype=[" + unitype + "]");
	...
	private void getParams(){
		...
		if(!Util.isEmpty(typeId) && (typeId.startsWith("k") || typeId.startsWith("q"))){
			...
			if(queryTypeId.startsWith("k"))
				ctClass = "0";
			else if(queryTypeId.startsWith("q"))
				ctClass = "1";
		}
		if(!Util.isEmpty(pageNum) && pageNum.length()>1){
		...
		if(!Util.isEmpty(areaId) && areaId.length()>1){
			areaId = areaId.trim();
			queryCityId = areaId;
		...
		//jobdate
		if(!Util.isEmpty(jobdate) && jobdate.length()>1 && jobdate.startsWith("d")){
	...
	public String rencai(){
		...
		// 获取根类型或子类型
		if("1".equals(ctClass)){
			if(typeId != null && !"".equals(typeId) && typeId.length() > 1){
				childTypeList = jobTypeDAO.getTopAll(typeId);
			}else{
				childTypeList = jobTypeDAO.getTopAll();
			}
	...
	public String oldSearch(){
		...
		if(!Util.isEmpty(ctClass) && ctClass.startsWith("k") && ctClass.length()>1){
			ctClass = ctClass.trim().substring(1);
			if("0".equals(ctClass))
				return jianzhi();
			else if("1".equals(ctClass))
				return job();
		}
		return search();
	}
