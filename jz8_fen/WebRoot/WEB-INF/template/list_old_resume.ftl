...
<body>
	...
	<div class="current">
		...
		<a href="/resume/${Parameters["typeId"]}">${city.simpleName}<#if typeId?exists && typeId.length()==3>${typemap[typeId]}</#if><#if queryTypeId?exists && queryTypeId.length()==5>${typemap[queryTypeId]}</#if>简历中心</a>
		<#if areaId?? && areaId!=''>
	...
	<div class="con_top">
		<div class="con_top_top">
			<p>类别：
			<a href="/resume/${Parameters["areaId"]}${Parameters["jobdate"]}${Parameters["key"]}" style="color:black;font-weight:600;">全部</a>
			<a href="/rs${Parameters["areaId"]}/k/" <#if ctClass?? && ctClass=='0'>class="curr_sel"</#if>>兼职</a>
			<a href="/rs${Parameters["areaId"]}/q/" <#if ctClass?? && ctClass=='1'>class="curr_sel"</#if>>全职</a>
			</p>
			<p>职位：
...
<div class="friendly_link">
	...
	<p><span>周边<#if queryTypeId?exists && queryTypeId.length()>1>${typemap[queryTypeId]}</#if>简历：</span>
