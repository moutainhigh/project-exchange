...
<body>
	...
	<div class="current">
		...
		<a href="/rencai/${Parameters["typeId"]}">${city.simpleName}个人求职<#if typeId?exists && typeId.length()==3>${typemap[typeId]}</#if><#if queryTypeId?exists && queryTypeId.length()==5>${typemap[queryTypeId]}</#if></a>
		<#if areaId?? && areaId!=''>
	...
	<div class="con_top">
		<div class="con_top_top">
			...
			<a href="/rc${Parameters["areaId"]}/k${Parameters["jobdate"]}/" <#if ctClass?? && ctClass=='0'>class="curr_sel"</#if>>兼职</a>
			<a href="/rc${Parameters["areaId"]}/q${Parameters["jobdate"]}/" <#if ctClass?? && ctClass=='1'>class="curr_sel"</#if>>全职</a>
			</p>
			<p>职位：
			<a href="/rencai/${Parameters["areaId"]}${Parameters["jobdate"]}${Parameters["key"]}" style="color:black;font-weight:600;">全部</a>
...
<div class="friendly_link">
	...
	<p><span>周边<#if queryTypeId?exists && queryTypeId.length()>1>${typemap[queryTypeId]}</#if>求职：</span>
