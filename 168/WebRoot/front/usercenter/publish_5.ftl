					...
					<form style="margin: 0pt;" method="post" name="checkForm" id="checkForm" enctype="multipart/form-data" action="${base}/user/publish/publish.htm">
						...
						<div class="a-shuxing">
							...
							<ul>
								...
								<li class="a-shuxing-value">
									<input id="phone" name="info.tel" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))" onkeyup="value=value.replace(/[^\d]/g,'') " value="<#if userObj.contact??>${userObj.contact.tel!''}</#if>">
