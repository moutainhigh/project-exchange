...
public class UserDaoImpl extends BaseDaoImpl implements UserDao {
	...
	public PageBean<User> getAllUsers(int pageIndex) {
		...
		List<User> list = s.createQuery(hql).setMaxResults(20).setFirstResult(startIndex).list();
	...
	public PageBean<User> queryUsersByKey(String key, int pageIndex){
		...
		page.setTotalRow(count.intValue());// 总记录数目
		Session s = this.getHibernateTemplate().getSessionFactory().getCurrentSession();
		List<User> list = s.createQuery(hql).setParameter(0, "%"+key+"%").setMaxResults(20).setFirstResult(startIndex).list();
		page.setResultList(list);// 数据列表
		page.setRowPerPage(20);// 每页记录数目
		page.setPageIndex(pageIndex);// 当前页码
		return page;
