...
public class RegionDAO extends HibernateDaoSupport  {
	...
	public List<Region> getCityListByProvince(Region pro){
		String hql = "from Region c where c.cityCode like ? and c.cityId!=? and length(c.cityCode)=4";
		return this.getHibernateTemplate().find(hql, new Object[]{pro.getCityCode().subSequence(0, 2) + "%", pro.getCityId()});
	}
	
	public List<Region> getCityListByBrother(Region city){
		String hql = "from Region c where c.parentCode=? and c.cityId!=?";
		return this.getHibernateTemplate().find(hql, new Object[]{city.getParentCode(), city.getCityId()});
	}
	
	public List<Region> getSubCityList(Region city){
    ...
    public Region findById( java.lang.String id) {
        ...
        try {
            Region instance = (Region) getHibernateTemplate().get("Region", id);
            return instance;
        } catch (RuntimeException re) {
