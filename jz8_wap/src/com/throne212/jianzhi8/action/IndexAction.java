...
public class IndexAction extends ActionSupport {
	...
	private Region city;
	private List<Type> indexTypeList;
	private List<Content> indexK01List;
	private List<Content> indexK02List;
	private List<Content> indexLastList;
			...
			if (path != null) {
				if (path.contains("/")) {
					...
					if (strpath.length == 2) { // 一级目录的时候
						str[0] = strpath[1];
						city = Parameter.citymap_py.get(str[0]);
						if(city == null)
							return "404";
						return index();
						...
						str[1] = strpath[2];
						str[2] = strpath[3];
					} else if (strpath.length == 5) {// 四级目录的时候
						str[0] = strpath[1];
						str[1] = strpath[2];
						str[2] = strpath[3];
						str[3] = strpath[4];
					...
					//根据标识调用不同的列表方法
					if(strParam[0].startsWith("zp") || strParam[0].equals("job")){//全职
						return job();
					}else if(strParam[0].startsWith("rc") || strParam[0].equals("rencai")){//求职
						return rencai();
					}else{//兼职
						return jianzhi();
					}
	...
	private void getParams() {
		...
		if(!Util.isEmpty(paramCityId)){
			Region r = Parameter.citymap_py.get(paramCityId);
			if(city == null)
				city = r;
			if(r != null){
			...
			queryCityCode = city.getCityCode();
			currCityId = city.getCityId();
			currCityName = city.getSimpleName();
			currParentCityId = city.getCityId();
			currParentCityName = city.getSimpleName();
		}
		//获取type的code和处理curr
	...
	//城市首页
	public String index(){
		indexTypeList = typeDAO.getIndexTypes();
		indexK01List = contentDAO.findLatestContent(city.getCityCode(), "k01", "1","0",8);
		indexK02List = contentDAO.findLatestContent(city.getCityCode(), "k02", "1","0",8);
		indexLastList = contentDAO.findLatestContent(city.getCityCode(), null, "1",null,8);
		//操作cookie
		Cookie c = new Cookie("jz8_wap_city_id",city.getCityId());
		c.setMaxAge(60*60*24*7);//一周时间
		c.setPath("/");
		ServletActionContext.getResponse().addCookie(c);
		logger.debug("add cookie:" + c);
		return "index";
	}
	...
	public List<Type> getIndexTypeList() {
		return indexTypeList;
	}

	public void setIndexTypeList(List<Type> indexTypeList) {
		this.indexTypeList = indexTypeList;
	}

	public List<Content> getIndexK01List() {
		return indexK01List;
	}

	public void setIndexK01List(List<Content> indexK01List) {
		this.indexK01List = indexK01List;
	}

	public List<Content> getIndexK02List() {
		return indexK02List;
	}

	public void setIndexK02List(List<Content> indexK02List) {
		this.indexK02List = indexK02List;
	}
	...
	public List<Content> getIndexLastList() {
		return indexLastList;
	}

	public void setIndexLastList(List<Content> indexLastList) {
		this.indexLastList = indexLastList;
	}
