...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function deleteInfo(){
			if(confirm('本操作就导致低收入户的相关资料被删除并不可恢复，您确定删除吗？') && $('input:checked').length>0){
				...
				<tbody>
					<tr>
						...
						<td align="right">
							...
							<c:if test="${not userObj.is206 || userObj.roleType=='超级管理员'}">
				...
				<tbody>
					...
					<c:forEach items="${pageBean.resultList}" var="f">
						<tr <c:if test="${f.type==5}">style="background: pink"</c:if>>
							...
							<td height="25" align="center" class="tables_contentcell">
								<c:if test="${not f.cun.is206 || userObj.roleType=='超级管理员'}">
