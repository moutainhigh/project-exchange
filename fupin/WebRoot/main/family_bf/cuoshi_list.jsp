...
var msg = '${msg}';
		...
		function deleteInfo(){
			if(confirm('您确定删除吗？') && $('input:checked').length>0){
				document.forms[0].action = '${appPath}cun_bf_deleteCuoshiFamily.action';
...
<td width="6%" class="tables_headercell">
				...
				<td height="25" align="center" class="tables_contentcell">	${f.id }</td>
				    ...
				    <c:if test="${f.status=='未提交'}">
						<a href="#" onclick="winOpen('${appPath}family_bf_confirmCuoshi.action?cuoshi.id=${f.id}',600,390);">确认后提交</a>
					</c:if>
					<c:if test="${f.status!='未提交'}">
						${f.status }
					</c:if>
