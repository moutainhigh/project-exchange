...
var msg = '${msg}';
		...
		function deleteInfo(){
			if(confirm('您确定删除吗？') && $('input:checked').length>0){
				document.forms[0].action = '${appPath}cun_bf_deleteChengxiaoFamily.action';
	...
	<div style="text-align: right;" class="content">
		<div class="p">
			<ul type="square">
			 <li>
			 <span>
               贫困户姓名：<input type="text" style="width: 90px;" value="" name="chengxiao.family.name" id="textfield"> 
<input type="submit" id="a11" value="查询" name="a11" class="button">
&nbsp;&nbsp;|&nbsp;&nbsp;
<input type="button" onclick="${appPath}family_bf_chengxiaoFamilyList.action" id="b11" title="清除条件并重新查询" value="清除条件，返回全部" name="b11" class="button">
	 </span>
			 </li>	 
			 <li>
			 <span>
...
<td width="6%" class="tables_headercell">
				...
				<td height="25" align="center" class="tables_contentcell">	${f.id }</td>
				    ...
				    <c:if test="${f.status=='未提交'}">
						<a href="#" onclick="winOpen('${appPath}family_bf_confirmChengxiao.action?chengxiao.id=${f.id}',600,390);">确认后提交</a>
					</c:if>
					<c:if test="${f.status!='未提交'}">
						${f.status }
					</c:if>
