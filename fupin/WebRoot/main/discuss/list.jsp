			...
			<table width="100%" cellspacing="0" cellpadding="0" border="0" class="tables_search">
				<tbody>
					<tr>
						...
						<td align="right">
						<input type="button" onclick="winOpen('${appPath}dis_showPublish.action',600,600);" class="button" value="发布新帖">
				...
				<tbody>
					...
					<c:forEach items="${pageBean.resultList}" var="f">
						<tr>
							...
							<td height="25" align="center" class="tables_contentcell">
							<c:if test="${f.lastReplyDate==null||f.lastReplyDate==''}">
							暂无回复
							</c:if>
							<c:if test="${f.lastReplyDate!=null&&f.lastReplyDate!=''}">
							<fmt:formatDate value="${f.lastReplyDate}" pattern="yyyy-MM-dd hh:mm"/> 
							</c:if>
							</td>
							<td height="25" align="center" class="tables_contentcell">
							
								<a href="${appPath}dis_showDiscussReply.action?discuss.id=${f.id }"> 
									查看
								</a>
