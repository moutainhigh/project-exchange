...
var msg = '${msg}';
		...
		function pass(){
			if(confirm('您确定通过审核吗？') && $('input:checked').length>0){
				document.forms[0].action = '${appPath}shenhe_passAllPicZhen.action';
				document.forms[0].submit();
			}
		}
			function notpass(){
			if(confirm('您确定不通过审核吗？') && $('input:checked').length>0){
				document.forms[0].action = '${appPath}shenhe_notpassAllPicZhen.action';
				document.forms[0].submit();
			}
		}
...
<form method="get" action="${appPath}shenhe_showAllPicZhenInPro.action" name="searchForm">

<table width="100%" cellspacing="0" cellpadding="0" border="0" class="tables_search">
	<tbody><tr>
	<td>您当前所处页面：审核&gt;&gt;镇图片审核 </td>
	<td align="right">
...
<table width="100%" cellspacing="0" cellpadding="0" border="0" class="tables_table">
	...
	<c:forEach items="${pageBean.resultList}" var="f">
		...
		<td height="25" align="center" class="tables_contentcell">
		<a href="${appPath}shenhe_passPicZhen?picZhen.id=${f.id}" >通过</a>
		<a href="${appPath}shenhe_notpassPicZhen.action?picZhen.id=${f.id}" >不通过</a>
