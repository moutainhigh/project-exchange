...
var msg = '${msg}';
		...
		function pass(){
			if(confirm('您确定通过审核吗？') && $('input:checked').length>0){
				document.forms[0].action = '${appPath}shenhe_passAllChengxiaoZhen.action';
				document.forms[0].submit();
			}
		}
			function notpass(){
			if(confirm('您确定不通过审核吗？') && $('input:checked').length>0){
				document.forms[0].action = '${appPath}shenhe_notpassAllChengxiaoZhen.action';
				document.forms[0].submit();
			}
		}
...
<form method="get" action="${appPath}shenhe_showAllChengxiaoZhenInPro.action" name="searchForm">

<table width="100%" cellspacing="0" cellpadding="0" border="0" class="tables_search">
	<tbody><tr>
	<td>您当前所处页面：审核&gt;&gt;镇帮扶成效审核 </td>
	<td align="right">
...
<table width="100%" cellspacing="0" cellpadding="0" border="0" class="tables_table">
	...
	<tbody><tr align="center">
		...
		<td width="20%" class="tables_headercell">审核</td>
	...
	<td height="25" align="center" class="tables_contentcell">
		<a href="${appPath}shenhe_passChengxiaoZhen?chengxiaoZhen.id=${f.id}" >通过</a>
		<a href="${appPath}shenhe_notpassChengxiaoZhen.action?chengxiaoZhen.id=${f.id}" >不通过</a>
		</td>
		</tr>
			</c:forEach>
	<tr>
