...
<head>
    ...
    <c:choose>    	<c:when test="${not empty special.seoTitle}">    		<c:set value="${special.seoTitle}" var="t"></c:set>    	</c:when>    	<c:otherwise>    		<c:set value="${appTitle}" var="t"></c:set>    	</c:otherwise>    </c:choose>    <c:choose>    	<c:when test="${not empty special.seoDesc}">    		<c:set value="${special.seoDesc}" var="d"></c:set>    	</c:when>    	<c:otherwise>    		<c:set value="${appDesc}" var="d"></c:set>    	</c:otherwise>    </c:choose>    
    <title>${t}</title>    
...
<div class="wrapper mauto pad_5">
      <div class="search_area">
        <div class="fl ft14 w550 hide">张家口汽车网 <a href="${appPath}/index.htm">首页</a> &gt; <a href="${appPath}/specialList.htm">本地优惠信息</a> &gt; <a href="#">${special.title }</a></div>
