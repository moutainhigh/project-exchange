...
public class UserDAO extends HibernateDaoSupport  {
	...
	public List<User> findLatestResume(final String cityId, final String typeCode, final int rows) {

		String hql = "from User where userCityId=? and userKind='0' and userIscheck<>'0' and userIslock<>'1'";
		if (typeCode != null && !"".equals(typeCode)) {
			String[] arr = typeCode.split("-");
			hql += " and (1!=1";
			for (String k : arr) {
				hql += " or userTypeId like '%" + k.trim() + "%'";
			}
			hql += ")";
		}
		hql += " order by userUpdate desc";
