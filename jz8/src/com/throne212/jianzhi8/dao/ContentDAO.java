...
public class ContentDAO extends HibernateDaoSupport {
	...
	public List<Content> findLatestContent(final String cityId, final String typeCode, final String kind, final int rows, boolean isHot) {
		...
		String hql = "from Content c where c.ctCityId=? and c.ctKind='"+kind+"' and ctIscheck='1' and ctIsclose='0'";
		if (typeCode != null && !"".equals(typeCode)) {
			String[] arr = typeCode.split("-");
			hql += " and (1!=1";
			for (String k : arr) {
				hql += " or c.ctTypeId like '%" + k.trim() + "%'";
			}
			hql += ")";
		}
		if(isHot){
			...
			hql += " order by ctIspay desc, ctIsyz desc, ctEnddate desc";
		}
