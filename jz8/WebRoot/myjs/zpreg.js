...
function CheckUserID(){
  var userid=document.zpRegForm.username.value;
    if (!checkUserName(document.zpRegForm.username))
    {
        document.zpRegForm.username.focus();
        return (false);
    }
    else
    {
    ...
    Trim_char(obj1);
        if(obj1.value!=""){
         obj2.style.display="none";
             obj2.innerHTML ="";
             document.all.fb_title22.innerHTML ="";
         }
         else{
           obj2.style.display="none";
             //obj2.innerHTML ="标题不能为空。";
            document.all.fb_title22.innerHTML ="标题不能为空。";
           }
  }
...
function f_check_ZhOrNumOrLett2(obj){    //判断用户名是否是汉字、字母、数字组成
  ...
  function check_con(obj){
    ...
    document.getElementById("con1").style.display="inline";
//    document.getElementById("con1").innerHTML ="<img src='../images/good2.gif'>&nbsp;<font color='#4DA531'>填写正确</font>";
    }
...
function check_phone1(obj){
  ...
  if(obj.value !=""){
    ...
    msg="<img src='/myimages/good2.gif'>&nbsp;<font color='#4DA531'>填写正确</font>";
    document.getElementById("phone11").style.display="none";
   }
...
//         document.getElementById("email2").innerHTML ="<img src='../images/good2.gif'>&nbsp;<font color='#4DA531'>填写正确</font>";
         ...
         if(document.zpRegForm.contact.value.length==25){
         document.zpRegForm.contact.value=document.zpRegForm.contact.value+obj.value;}
         textCounter1(100);
         return true;
}
...
//return true;
   ...
   if (!isemail(form.email.value) )
   {
     ...
     Trim_char(form.unitName);
  if (form.unitName.value.length <2)
    {
        alert("请填写单位名称！");
        form.unitName.focus();
        return (false);
    }
    if(countInstances(form.unitName.value)>0){
    alert("信息监控系统提醒您：很抱歉，由于您提交的内容中含有系统不允许的关键词或者您的IP受到了访问限制，本次操作无效，系统已记录您的IP及您提交的所有数据。请注意，不要提交任何违反国家规定的内容！");
    form.unitName.focus();
      return (false);
    }
     Trim_char(form.address);
    ...
    if(countInstances(form.address.value)>0){
    alert("信息监控系统提醒您：很抱歉，由于您提交的内容中含有系统不允许的关键词或者您的IP受到了访问限制，本次操作无效，系统已记录您的IP及您提交的所有数据。请注意，不要提交任何违反国家规定的内容！");
    form.address.focus();
      return (false);
    }
  ...
  if (form.contact.value.length <5)
    ...
    Trim_char(form.unitIntro);
     if (form.unitIntro.value.length <5)
    {
        alert("请填写单位简介！\n字符数5-500");
        form.unitIntro.focus();
        return (false);
    }
        if (strangecode(form.unitIntro.value)==false)
    {
        alert("\n单位简介不能含有特殊字符 < >");
        form.unitIntro.focus();
        return (false);
    ...
    return false;
     ...
     if(countInstances(form.unitIntro.value)>0){
    alert("信息监控系统提醒您：很抱歉，由于您提交的内容中含有系统不允许的关键词或者您的IP受到了访问限制，本次操作无效，系统已记录您的IP及您提交的所有数据。请注意，不要提交任何违反国家规定的内容！");
    form.unitIntro.focus();
      return (false);
    }
...
//        return (false);
//   	}
//   if (strangecode(form.salary.value)==false)
//    {
//        alert("\n薪资待遇中不能含有特殊字符 < >");
//        form.salary.focus();
//        return (false);
...
//    }
    ...
    if (getLength5(form.yzcode.value)!=4)
    {
        alert("验证码位数不对！");
        form.yzcode.focus();
        return (false);
