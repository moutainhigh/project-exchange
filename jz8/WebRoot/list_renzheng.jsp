...
<jsp:include page="nav.jsp"></jsp:include>
			...
			<DIV id=search2>
				<DIV id=searchBox2>
					...
					<A href="/">${city.simpleName}兼职网</A>&nbsp;>&nbsp; 兼职工作

					<font color='#7f7f7f'>(${bean.totalRow })</font>
				</DIV>
				<div style='float: right; font-color: red;'>
					<font color="red"> 本栏目所有兼职工作都已承诺不收费，请大家放心求职，有任何疑问可以联系本站！ </font>
				</div>
			</DIV>
			<!-- search end -->
			<DIV id=content>
	...
	new google.ads.search.Ads(pageOptions, options1);
					...
					<DIV class=listInfo>
						<table width="100%" id="listTbl" cellpadding="0" cellspacing="2"
							...
							<tr>
								<td width='650'>
									<font color="#000000"><c:if test="${c.ctClass=='0'}">兼职</c:if><c:if test="${c.ctClass=='1'}">全职</c:if>-</font>&nbsp;
					...
					<div class=p>
						<div style='float: right'>
							...
							<c:if test="${!bean.isFirstPage}">
								<a href="/renzheng/${typeId}c${ctClass}p1">首页</a>
								<a href="/renzheng/${typeId}c${ctClass}p${bean.prePageIndex}" class="pre">上页</a>
							</c:if>
							${bean.pagerRenzheng }
							...
							<c:if test="${!bean.isLastPage}">
								<a href="/renzheng/${typeId}c${ctClass}p${bean.nextPageIndex}" class="nxt">下页</a>
							</c:if>
   ...
   document.getElementById("${city.cityCode}").style.background="#004080";
				...
				<DIV style="margin-top: 1px;">
					<P
						...
						兼职吧提醒您:
						网上信息有风险,本站所有${city.simpleName}兼职工作在线招聘信息均由网友自由发布，请仔细审核，兼职吧招聘网不承担由于内容的合法性及真实性所引起的一切争议和法律责任！
					</P>
				</DIV>
